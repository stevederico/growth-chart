...
/**
 * Validate parsed config.json matches the expected raw shape.
 *
 * Narrows the unknown JSON value read from config.json before it is trusted
 * as a BackendConfig source: requires a `database` object with string `db`,
 * `dbType`, and `connectionString` fields, and an optional string `staticDir`.
 *
 * @param value - Parsed JSON value from config.json
 * @returns True if the value has the expected raw config shape
 */
